...
fn main() {
    ...
    let result = match subcommand {
        ...
        "verify" => run_verify(),
        "gen-verifier" => run_gen_verifier(),
        "version" => {
            println!("basis-prover v0.1.0");
            Ok(())
        }
        _ => {
            eprintln!("Usage: basis-prover <witness|prove|verify|aggregate|gen-verifier|version>");
            eprintln!();
            ...
            eprintln!("  gen-verifier  Generate Halo2-compatible Solidity verifier (stdout: Solidity)");
...
/// Generate a Halo2-compatible Solidity verifier contract.
///
/// Outputs two Solidity contracts to stdout (verifier + VK) that can verify
/// proofs generated by this prover. Uses halo2-solidity-verifier (PSE) for
/// industry-standard Halo2 PLONK-KZG verification.
///
/// Usage: basis-prover gen-verifier > Halo2Verifier.sol
fn run_gen_verifier() -> Result<(), Box<dyn std::error::Error>> {
    let k = 8u32; // Match the circuit k used in prove

    eprintln!("[gen-verifier] Generating Solidity verifier (k={})...", k);
    let start = std::time::Instant::now();

    let (verifier_sol, vk_sol) = basis_evm_verifier::generate_solidity_verifier(k)
        .map_err(|e| format!("verifier generation failed: {}", e))?;

    let elapsed = start.elapsed();
    eprintln!(
        "[gen-verifier] Generated in {}ms (verifier: {} bytes, vk: {} bytes)",
        elapsed.as_millis(),
        verifier_sol.len(),
        vk_sol.len()
    );

    // Output VK contract first, then verifier
    println!("// ==========================================================");
    println!("// Halo2Vk.sol -- Generated VK contract for Basis Network circuit");
    println!("// Generated by: basis-prover gen-verifier (halo2-solidity-verifier)");
    println!("// ==========================================================");
    println!();
    print!("{}", vk_sol);
    println!();
    println!("// ==========================================================");
    println!("// Halo2Verifier.sol -- Generated verifier for Basis Network circuit");
    println!("// Generated by: basis-prover gen-verifier (halo2-solidity-verifier)");
    println!("// ==========================================================");
    println!();
    print!("{}", verifier_sol);
