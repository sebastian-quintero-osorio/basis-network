...
///
///      Extends the validium StateCommitment.sol (RU-V3) patterns:
///        - INV-S1 ChainContinuity: prevRoot must match currentRoot before finalization
///        - INV-S2 ProofBeforeState: ZK proof verified before state mutation
///        - NoGap: batch IDs auto-incremented per enterprise
///        - EnterpriseIsolation: per-enterprise mappings with msg.sender enforcement
///
///      New rollup invariants:
///        - INV-R1 SequentialExecution: batches executed in strict order
...
///        - INV-R5 RevertSafety: only unexecuted batches can be reverted
contract BasisRollup {
    ...
    /// @dev Groth16 verifying key (same structure as StateCommitment.sol).
    struct VerifyingKey {
        uint256[2] alfa1;
        uint256[2][2] beta2;
        uint256[2][2] gamma2;
    ...
    /// @dev Stored on-chain per batch. Uses batch hash pattern (1 slot for hash + 1 slot for packed metadata).
    ...
    ///      INV-R4 MonotonicBlockRange: l2BlockStart must equal lastL2Block + 1 (or 1 if first batch).
    ///      NoGap: batchId is auto-incremented from totalBatchesCommitted.
    ...
    function commitBatch(CommitBatchData calldata data) external {
        ...
        // Store batch info (3 storage slots)
    ...
    /// @dev INV-R3 CommitBeforeProve: batch must be in Committed status.
    ///      INV-S2 ProofBeforeState: proof verified before any state change.
    ///      Proves batches sequentially (totalBatchesProven must match batchId).
    ...
    function proveBatch(
        ...
        // Sequential proving: must prove in order
        ...
        // INV-S2: Verify proof BEFORE any state change
        bool valid = _verifyProof(a, b, c, publicSignals);
        if (!valid) revert InvalidProof();

        // Transition batch to Proven
    ...
    /// @dev INV-R2 ProveBeforeExecute: batch must be in Proven status.
    ///      INV-R1 SequentialExecution: batches executed in strict order.
    ///      INV-S1 ChainContinuity: verified implicitly (batches committed in order,
    ///      each batch's stateRoot was committed against the chain at commit time).
    ...
    /// @dev INV-R5 RevertSafety: only unexecuted batches can be reverted.
    ///      Reverts from the most recently committed batch backward.
    ///      If the batch was proven, it is also unproven.
    ...
    function revertBatch(address enterprise) external onlyAdmin {
        ...
        // Cannot revert executed batches
