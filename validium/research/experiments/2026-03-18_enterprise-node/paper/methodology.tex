...
\begin{enumerate}
\item[\textbf{INV-NO1}] \emph{Liveness}: If $|\text{pendingTx}| > 0$ and
  $s = \text{Idle}$, then eventually $s = \text{Proving}$.

\item[\textbf{INV-NO2}] \emph{Safety}: If $s = \text{Submitting}$, then
  $\text{proof.prevRoot} = \text{smt.prevRoot}$ and
  $\text{proof.newRoot} = \text{smt.currentRoot}$.

\item[\textbf{INV-NO3}] \emph{Privacy}: The only data transmitted outside
...
\item[\textbf{INV-NO4}] \emph{Crash Recovery}: After crash and restart,
  $|\text{walReplayed}| + |\text{committed}| = |\text{totalEnqueued}|$.

\item[\textbf{INV-NO5}] \emph{State Root Continuity}: For batch $N$,
  $\text{submit}(N).\text{prevRoot} = \text{batch}(N-1).\text{newRoot}$.

\item[\textbf{INV-NO6}] \emph{Single Writer}: Only the batch loop
